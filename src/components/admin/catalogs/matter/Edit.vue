...
<q-card>
  <q-card-section>
    <div class="text-h6">Actulizar Materia</div>
    <br>
    <q-form ref="project_form" @submit.prevent="() => {}">
      <div class="row q-col-gutter-sm q-mb-md">
        <div class="col-xs-12 col-sm-6 col-md-6">
          <q-input label="Nombre de la Materia" v-model="form.name" round outlined :rules="[(val) => !!val || 'Este campo es obligatorio']"/>
        </div>
      </div>
    </q-form>
...
export default {
  ...
  created () {
    ...
    this.$store.dispatch('catalogs/setCatalogs').then(() => {
      MatterService.edit(id).then((data) => {
        this.form = data
        this.$q.loading.hide()
      }).catch(() => {
        this.$q.loading.hide()
  ...
  methods: {
    update () {
      ...
      const { id } = this.$route.params
      MatterService.update(form, id).then(() => {
