...
/* La nomenclatura sugerida es la siguiente
  ...
  * */
const basePath = '/api/matter'

export const index = (payload) => {
  const URL = basePath
  return axiosInstance.get(URL, payload).then((res) => res.data.matter)
}

export const edit = (id) => {
  const URL = `${basePath}/${id}/edit`
  return axiosInstance.get(URL).then((res) => res.data.matter)
}

export const destroy = (id) => {
  const URL = `${basePath}/${id}`
  return axiosInstance.delete(URL).then((res) => res.data.matter)
}
